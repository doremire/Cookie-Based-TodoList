...
## ⚡Trigger
外出中やアプリを開きたくない時、それほど大切じゃないことを書き込みたい時ってありますよね!!
アプリはアプリストアを開きインストールしなきゃいけない、どうせ数回しか使わないアプリを入れて、スマホの容量を圧迫する。
そんな事せずWEBで完結する物がほしい！！
他社のサービスはWEBで完結するけど、数回しか使わないのにアカウント登録するのは億劫!!
そんなあなたの為に`アカウント登録不要` `WEBで完全完結` [詳しくは注意事項を参照] で、できる物を開発しました！！

注意事項

1. 本サービスはCookie（クッキー）、スマホやPC内のブラウザに保存される情報、入力したデータ、などを記録する仕組みを利用する為、Cookieを意図的にブロックする、Cookieをブロックするブラウザなどを使用されますと、システムが正常に機能しない恐れがあります。
詳しくは [Cookieのポリシー](https://doremire-server.com/policy/) をご覧ください。

1. 保存情報がある状態でCookieを削除されますと保存情報が削除されますのでご注意ください。
